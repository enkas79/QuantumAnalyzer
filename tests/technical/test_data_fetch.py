...
from quantumanalyzer.technical import data
...
def test_fetch_ohlcv_falls_back_to_stooq_when_yahoo_empty():
    with (
        patch("quantumanalyzer.technical.data.yf.download", return_value=pd.DataFrame()),
        patch("quantumanalyzer.technical.data._download_stooq_csv", return_value=_STOOQ_CSV) as mock_stooq,
    ):
...
def test_fetch_ohlcv_raises_combined_error_when_both_sources_fail():
    with (
        patch("quantumanalyzer.technical.data.yf.download", side_effect=RuntimeError("yahoo down")),
        patch("quantumanalyzer.technical.data._download_stooq_csv", side_effect=RuntimeError("stooq down")),
    ):
