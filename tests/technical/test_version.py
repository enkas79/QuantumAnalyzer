...
from quantumanalyzer.technical import _read_version


def test_read_version_uses_installed_package_metadata():
    with patch("quantumanalyzer.technical._pkg_version", return_value="1.2.3"):
...
def test_read_version_falls_back_to_bundled_version_txt_when_metadata_missing(tmp_path):
    ...
    with (
        patch("quantumanalyzer.technical._pkg_version", side_effect=PackageNotFoundError("quantumanalyzer")),
        patch("quantumanalyzer.technical.sys") as mock_sys,
    ):
