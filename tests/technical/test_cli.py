...
from quantumanalyzer.technical.cli import main
...
def _patched(**overrides):
    ...
    return (
        patch("quantumanalyzer.technical.cli.resolve_ticker", return_value=defaults["resolve_ticker"]),
        patch("quantumanalyzer.technical.cli.fetch_ohlcv", return_value=defaults["fetch_ohlcv"]),
        patch("quantumanalyzer.technical.cli.analyze", return_value=defaults["analyze"]),
    )
...
def test_cli_reports_error_and_returns_one_on_failure(capsys):
    with patch("quantumanalyzer.technical.cli.resolve_ticker", side_effect=ValueError("ticker mancante")):
        exit_code = main([""])
