...
def test_check_for_update_returns_none_when_already_latest():
    ...
    with patch("quantumanalyzer.technical.updater.urllib.request.urlopen", return_value=_fake_response(payload)):
...
def test_check_for_update_returns_none_on_network_failure():
    with patch("quantumanalyzer.technical.updater.urllib.request.urlopen", side_effect=OSError("no network")):
        assert check_for_update("0.2.0") is None


def test_check_for_update_returns_none_on_malformed_payload():
    with patch("quantumanalyzer.technical.updater.urllib.request.urlopen", return_value=_fake_response({})):
        assert check_for_update("0.2.0") is None
