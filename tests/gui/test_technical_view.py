...
def test_analyze_flow_updates_result_panel_and_chart(qtbot):
    ...
    with (
        patch("quantumanalyzer.gui.technical_workers.resolve_ticker", return_value=("AAPL", "Apple Inc.")),
        patch("quantumanalyzer.gui.technical_workers.fetch_ohlcv", return_value=df),
        patch("quantumanalyzer.gui.technical_workers.analyze", return_value=result),
    ):
...
def test_watchlist_run_populates_table(qtbot):
    ...
    with (
        patch(
            "quantumanalyzer.gui.technical_workers.resolve_ticker",
...
def test_watchlist_run_reports_errors_without_stopping(qtbot):
    ...
    with patch("quantumanalyzer.gui.technical_workers.resolve_ticker", side_effect=ValueError("boom")):
        window._on_watchlist_run()
...
def test_check_for_updates_runs_off_the_ui_thread_and_reports_back(qtbot):
    ...
    with (
        patch("quantumanalyzer.gui.technical_workers.check_for_update", return_value=info),
...
def test_load_selected_watchlist_row_button(qtbot):
    ...
    with (
        patch("quantumanalyzer.gui.technical_workers.resolve_ticker", return_value=("MSFT", "Microsoft Corp.")),
        patch("quantumanalyzer.gui.technical_workers.fetch_ohlcv", return_value=df),
        patch("quantumanalyzer.gui.technical_workers.analyze", return_value=result),
    ):
        window._on_watchlist_run()
